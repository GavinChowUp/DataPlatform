...
from (select *, row_number() over (partition by product_id order by total_profit) as top_num
      from (select to_char(pa.order_date, 'YYYY-WW') as week_of_year,
            ...
            group by week_of_year, dp.product_id) as pd) as tn
where top_num <= 10;
